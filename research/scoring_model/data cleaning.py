...
def is_important(sentence):
    ...
    return (
            len(doc.ents) > 0 or
            3 < len(doc) < 30 or
            any(token.pos_ in {'PROPN', 'NUM'} for token in doc)
    )
...
def filter_and_transform(df):
    ...
    keywords = [
        'browser', 'menu', 'contact', '2020 antwerpen', 'sportcentrum', 'bel', 'surf', 'ook interessant',
        'locatie', ':', 'telefoneer', 'schrijf', 'website', 'aanbod', 'gezin', 'euro', 'mail', 'tel+', '@',
        '€', 'stadsplan', 'leaflet', 'gemeentearchief', 'cookie', 'NL', 'internetbrowser', 'E-mail', '©', '™'
    ]
    exclusion_pattern = '|'.join(keywords)

    all_sentences = [sentence for text in df['body_content'].dropna() for sentence in split_sentences(text)]
    ...
    filtered_df = filtered_df[
        ~(filtered_df['sentence'].str.contains(r'\|') & filtered_df['sentence'].str.contains('BE'))]
